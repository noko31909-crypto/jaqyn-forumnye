...
export interface DemoStore {
  ...
  toggleFavorite: (integrationId: number) => void;
  addActionHistory: (action: string, description: string, icon: string) => void;
  resetDemoData: () => void;
  getCustomersBySegment: (segment: Customer['status']) => Customer[];
  getTotalRevenue: () => number;
  getActiveCampaigns: () => Campaign[];
  getActiveTools: () => Integration[];
}
...
export const useDemoStore = create<DemoStore>()(
  persist(
    (set, get) => ({
      ...
      toggleFavorite: (integrationId) =>
        set((state) => ({
          favorites: state.favorites.includes(integrationId)
            ? state.favorites.filter(id => id !== integrationId)
            : [...state.favorites, integrationId]
        })),

      addActionHistory: (action, description, icon) =>
        set((state) => ({
          actionHistory: [
            { id: Date.now(), action, description, timestamp: new Date(), icon },
            ...state.actionHistory.slice(0, 19) // Keep last 20 actions
          ]
        })),
      ...
      getActiveTools: () =>
        get().integrations.filter((i) => i.isActive),
    }),
    {
